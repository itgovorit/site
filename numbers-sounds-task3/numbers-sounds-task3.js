...
import bgUrl from "../src/img/numbers-sounds/background3.jpg";
import purpleUrl from "../src/img/numbers-sounds/purple.png";
import yellowUrl from "../src/img/numbers-sounds/yellow.png";
import greenGemUrl from "../src/img/numbers-sounds/green.png";
import greenUrl from "../src/img/numbers-sounds/green-sm.png";
import rubyUrl from "../src/img/numbers-sounds/ruby-sm.png";

// This task drills the /θ/ vs /f/ vs /t/ contrast (three options).
const group = pairs.find((g) => g.group === "TH vs F vs T");

// Runestone image for each answer option (three contrasts → three gems).
const answerImages = [purpleUrl, yellowUrl, greenGemUrl];
...
function Explanation() {
  return `
    <section class="mx-auto max-w-3xl space-y-4 px-4 py-10 leading-relaxed">
      ...
      <p>
        Нажми <strong>Play</strong>, послушай слово и выбери звук, который слышишь
        в начале слова. Тут целых три варианта! Всего ${group.words.length} слов.
      </p>
    </section>
  `;
}
    ...
    .map(
      (sound, i) => `
        <button
          ...
          data-sound="${sound}"
        >
          <img src="${answerImages[i % answerImages.length]}" alt="" class="h-28 w-28 object-contain drop-shadow-xl sm:h-36 sm:w-36" />
          <span class="absolute inset-0 flex items-center justify-center text-4xl font-black text-white/70 [text-shadow:_0_0_10px_rgb(255_255_255_/_0.9),_0_0_22px_rgb(255_255_255_/_0.6)] [-webkit-text-stroke:2px_rgb(0_0_0_/_0.55)] [paint-order:stroke_fill] sm:text-5xl">${sound.replace(/\//g, "")}</span>
        </button>
      `,
    )
    .join("");
...
function App() {
  return `
    ${Header({ backHref: "/numbers-sounds/#task3" })}
    <main>
      ${Explanation()}
      ${Task()}
    </main>
