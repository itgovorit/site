...
import bgUrl from "../src/img/numbers-sounds/background2.jpg";
...
// This task drills the /v/ vs /f/ contrast (final voicing).
const group = pairs.find((g) => g.group === "V vs F");
  ...
  return `
    <section class="mx-auto max-w-3xl space-y-4 px-4 py-10 leading-relaxed">
      ...
      <p>
        ...
        в конце слова. Всего ${group.words.length} слов.
      </p>
    </section>
  `;
}
...
function App() {
  return `
    ${Header({ backHref: "/numbers-sounds/#task2" })}
    <main>
      ${Explanation()}
      ${Task()}
    </main>
